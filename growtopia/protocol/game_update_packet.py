...
class GameUpdatePacket(Packet):
    ...
    flags: list[GameUpdatePacketFlags]
    ...
    def __init__(
        ...
        *,
        ...
        self.flags: list[GameUpdatePacketFlags] = flags or []
    ...
    def set_variant_list(self, variant_list: VariantList) -> None:
        if GameUpdatePacketFlags.EXTRA_DATA not in self.flags:
            self.flags.append(GameUpdatePacketFlags.EXTRA_DATA)
    ...
    def serialise(self) -> bytes:
        ...
        if (GameUpdatePacketFlags.EXTRA_DATA not in self.flags) and self.extra_data:
            self.flags.append(GameUpdatePacketFlags.EXTRA_DATA)
        ...
        self.data += combined_flags.to_bytes(4, "little")
        ...
        if GameUpdatePacketFlags.EXTRA_DATA in self.flags:
    ...
    def from_bytes(cls, data: bytes) -> "GameUpdatePacket":
        ...
        flag_val = int.from_bytes(data[16:20], "little")
        ...
        if (GameUpdatePacketFlags.EXTRA_DATA in update_packet.flags) or len(data[56:]) > 0:
            if len(data) < 60:
